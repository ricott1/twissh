...
class MonsterStrategy(Strategy):
    ...
    def target(self):
        closest = sorted(
            [
                ent
                for ent in self.entity.location.characters
                if ent is not self.entity
                and isinstance(ent, self.target_type)
                and distance(self.entity.position, ent.position) < self.vision
            ],
            key=lambda ent: distance(self.entity.position, ent.position),
        )
        if closest:
            return closest[0]
        return None
