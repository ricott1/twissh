...
class HideCounter(Counter):
    ...
    def on_set(self):
        ...
        MarkerCounter(
            self.entity,
            {k: [" " for _ in self.entity.positions] for k in ("up", "down", "left", "right")},
            self.value,
        )

    def on_update(self, _deltatime):
        super().on_update(_deltatime)
        self.entity.recoil += _deltatime * (1 + Recoil.SHORT)
...
class DeathCounter(Counter):
    ...
    def on_set(self):
        ...
        MarkerCounter(
            self.entity,
            {k: ["X" for _ in self.entity.positions] for k in ("up", "down", "left", "right")},
            self.value,
        )
...
class PortalCounter(Counter):
    ...
    def __init__(self, _entity):
        super().__init__(_name="portal", _entity=_entity, _value=Recoil.SHORT)
...
class ParryCounter(Counter):
    ...
    def on_set(self):
        ...
        MarkerCounter(
            self.entity,
            {
                k: [v for _ in self.entity.positions]
                for k, v in zip(("up", "down", "left", "right"), ("◠", "◡", "(", ")"))
            },
            self.value,
        )
        ...
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                ...
                if self.entity.location.is_empty(pos):
                    entity.Song(
                        _spawner=self.entity,
                        _on_hit=self.on_song_hit,
                        _location=self.entity.location,
                        _position=pos,
                    )
