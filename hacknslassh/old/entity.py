...
from hacknslassh.utils import new_id, distance
...
class Entity(object):
    ...
    def hit(self, dmg):
        ...
        counter.ColorCounter(self, f"white", Recoil.SHORT)
        self.location.redraw = True
        if self.is_dead:
            self.destroy()
...
class Wall(Entity):
    def __init__(self, _HP, _name="wall", **kwargs):
        super().__init__(_name=_name, _extra_positions=[(0, 0, 1), (0, 0, 2)], _HP=_HP, **kwargs)
...
class IceWall(Wall):
    ...
    def __init__(self, _spawner=None, _marker="▓", **kwargs):
        self.spawner = _spawner
        _HP = 1 + 10 * (3 + self.spawner.INT.mod)
        super().__init__(_name="ice wall", _HP=_HP, _marker=_marker, _color="cyan", **kwargs)
        self.vanish = 0
        self.vanish_coeff = self.VANISH_COEFF * (1 - self.VANISH_SPAWNER_COEFF * self.spawner.INT.mod)
        self.med_damaged_marker = ["▒" for _ in self._marker]
        self.high_damaged_marker = ["░" for _ in self._marker]
    ...
    def marker(self):
        if self.HP.value > 0.75 * self.HP.max:
            ...
            return self.med_damaged_marker
        else:
            return self.high_damaged_marker
...
class SummonPortal(Entity):
    ...
    def status(self):
        _status = [
            (
                "top",
                f"{self.name:12s} {type(self.summon).__name__} {int(self.ritual_length-self.vanish)}",
            )
        ]
        return _status
...
class ActingEntity(Entity):
    def __init__(self, _movement_speed=BASE_MOVEMENT_SPEED, **kwargs):
        super().__init__(**kwargs)

        self._movement_speed = _movement_speed
...
class Projectile(ActingEntity):
    ...
    def on_update(self, _deltatime):
        ...
        if self.location.is_empty(self.forward):
            {"right": action.DashRight, "left": action.DashLeft, "up": action.DashUp, "down": action.DashDown,}[
                self.direction
            ].use(self)
...
class Arrow(Projectile):
    def __init__(self, _spawner, _on_hit, _dmg, _crit, _max_range=1, **kwargs):
        self.movement_recoil = Recoil.SHORT * (1 - MOD_WEIGHT * _spawner.DEX.mod)
        self.max_range = _max_range
        self.dmg = _dmg
        ...
        super().__init__(
            _spawner=_spawner,
            _on_hit=_on_hit,
            _movement_speed=5 + 0.5 * _spawner.DEX.mod,
            _max_range=_max_range,
            _direction=_spawner.direction,
            _position=_spawner.forward,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            _spawner=_spawner,
            _on_hit=_on_hit,
            _movement_speed=1.5 + 0.25 * _spawner.INT.mod,
        ...
        self._color = "red"
        self.movement_recoil = Recoil.MINIMUM * (1 - MOD_WEIGHT * _spawner.INT.mod)
