...
from hacknslassh.utils import log, mod, roll, random_name, random_stats, roll1d20, roll1d4
from hacknslassh.constants import *
from hacknslassh.characteristic import Characteristic
...
class Character(entity.ActingEntity):
    ...
    def status(self):
        ...
        recoil = (RECOIL_BARS_LENGTH - recoil_bars) * "▰" + recoil_bars * "▱"
        ...
        _status = [
            ("name", f"{self.name:12s}"),
            (recoil_type, f"{recoil}"),
            (self.color, f" {h}"),
            ("top", f"{self.HP.value:>3d}/{self.HP.max:<3d}"),
            ("top", f" {action_text:<s}"),
        ]
    ...
    def add_inventory(self, obj):
        if (
            self.inventory.has_free_spot()
            and EXTRA_ENCUMBRANCE_MULTI * self.encumbrance >= self.inventory.encumbrance + obj.encumbrance
        ):
