from hacknslassh.utils import roll1d20, roll1d4, roll
...
class Move(Action):
    ...
    def use(cls, user):
        ...
        if user.direction != cls.direction:
            ...
            user.movement_recoil += Recoil.MINIMUM / (user.movement_speed)
...
class Dash(Action):
    ...
    def use(cls, user):
        ...
        x, y, z = user.position
        user.position = (x + delta_x, y + delta_y, z)
        user.recoil += Recoil.MINIMUM
        user.movement_recoil += Recoil.MINIMUM / (DASH_SPEED_MULTI * user.movement_speed)
...
class Drop(Action):
    ...
    def requisites(cls, user, obj=None):
        ...
        x, y, z = user.position
        return (
            super().requisites(user)
            and user.location.is_empty((x, y, 0))
            and isinstance(obj, item.Item)
            and obj.id in user.inventory.entities
        )
...
class Attack(Action):
    ...
    def hit(cls, user, target):
        ...
        counter.MarkerCounter(
            user,
            {k: [v for _ in user.positions] for k, v in zip(("up", "down", "left", "right"), ("⩓", "⩔", "⪡", "⪢"))},
            Recoil.SHORT,
        )
        if r >= val:
...
class SneakAttack(Action):
    ...
    def use(cls, user):
        ...
        if isinstance(weapon, item.Bow):
            ...
            if (
                target.direction == user.direction
                and isinstance(weapon, item.Sword)
                and isinstance(target, character.Character)
            ):
                cls.hit(user, target)
            else:
...
class Parry(Action):
    ...
    def requisites(cls, user):
        ...
        return (
            super().requisites(user)
            and "parry" not in user.counters
            and isinstance(user.equipment["off_hand"], item.Shield)
        )
...
class Arrow(Action):
    ...
    def requisites(cls, user):
        ...
        return (
            super().requisites(user)
            and user.location.is_empty(user.forward)
            and isinstance(user.equipment["main_hand"], item.Bow)
        )
    ...
    def hit(cls, proj):
        ...
        if r >= proj.crit_range:
            ...
            counter.TextCounter(
                proj.spawner,
                f"Attack {target.name}: critical! {dmg} damage{'s'*int(dmg>1)}!",
            )
...
class FireBall(Action):
    ...
    def hit(cls, proj):
        ...
        if target:
            ...
            counter.TextCounter(
                proj.spawner,
                f"FIREBALL in {target.name}'s face: {dmg} damage{'s'*int(dmg>1)}!",
            )
        ...
        if proj.fragment > 1:
            ...
            for direction in extra_pos:
                ...
                if proj.location.is_empty(new_pos):
                    frag = entity.FireBall(
                        _spawner=proj.spawner,
                        _on_hit=cls.hit,
                        _direction=direction,
                        _position=new_pos,
                        _fragment=proj.fragment - 1,
                        _location=proj.location,
                    )
    ...
    def use(cls, user):
        if cls.requisites(user):
            proj = entity.FireBall(
                _spawner=user,
                _on_hit=cls.hit,
                _direction=user.direction,
                _position=user.forward,
                _fragment=user.INT.mod + 1,
            )
            user.recoil += cls.recoil_cost
            user.MP += cls.MP_cost
...
class Sing(Action):
    ...
    def hit(cls, song):
        ...
        counter.BuffCounter(
            target,
            cls.SONG_LENGTH + song.spawner.CHA.mod,
            "STR",
            max(1, song.spawner.CHA.mod),
        )
