...
class Action(object):
    ...
    def target_square(cls, user):
        ...
        return user.location.forward(user.position, user.direction)
...
class Attack(Action):
    ...
    def use(cls, user):
        ...
        if cls.is_legal(user, target):
            ...
            elif target.action_counters["parry"]>0 and target.direction == atk_direction:
...
class Fire(Action):
    ...
    def is_legal(cls, user):
        """Defines action legality"""
        forward = user.location.forward(user.position, user.direction)
        return super().is_legal(user) and user.location.is_empty(forward)

    @classmethod
    def use(cls, user):
        if cls.is_legal(user):
            proj = entity.Projectile(_spawner = user, _direction=user.direction, _position=user.location.forward(user.position, user.direction), _location=user.location)
