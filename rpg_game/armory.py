...
def warHammer(_location=None):
    return item.Hammer(_name="War Hammer", _range=2, _bonus={"DEX":-1}, _location=_location, _description="Ready to tear down", _dmg=(2, 4), _rarity="uncommon")
...
def chiappilArmor(_location=None):
    return item.Armor(_name="Chiappilarmor", _location=_location, _bonus={"DEX":4}, _dmg_reduction=0, _description="Vai vai...", _rarity="unique")
...
def rovaisThorn(_location=None):
    thorn = item.Sword(_name="Rovai\'s Thorn", _location=_location, _bonus={"STR":1, "INT": -1}, _description="Robbosi si nasce", _dmg=(2, 8), _rarity="set")
    thorn.set = {"name" : "Rovai\'s Gear", "size" : 2}
    thorn.set_bonus = {"STR":1, "INT": 3}
    return thorn

def rovaisFlipFlops(_location=None):
    flips = item.Boots(_name="Rovai\'s Flip Flops", _location=_location, _description="As comfy as they look", _rarity="set")
